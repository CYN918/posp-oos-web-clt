...
export class DeductibleRecord extends Component {
    ...
    public searchData = async () => {
        ...
        const {HEAD, BODY} = await indexService.FNC_003(this.searchParam);
        const {MSG, CODE} = HEAD;
        if (CODE === '000') {
    ...
    public render() {
        ...
        return (
            <Fragment>
                <Row>
                    <Col span={8}>
                        <Form.Item label="用户手机号" labelCol={{span: 6}} wrapperCol={{span: 16}}>
                            <Input placeholder="请输入手机号" onChange={this.handleChange('mobile')}/>
                        </Form.Item>
                    </Col>
                    <Col span={8}>
                        <Form.Item label="用户名称" labelCol={{span: 6}} wrapperCol={{span: 16}}>
