 ...
 */
export const parseParamToObj = (str: string) => {
    if (!str) {
        return {};
    }
...
export const getSerializedUrl = (url: string, requestData) => {
...
export const getHostUrl = (path) => {
    let hostUrl = (window as any).GLOBAL.hostUrl;
    if (hostUrl.lastIndexOf('/') === hostUrl.length - 1) {
        hostUrl = hostUrl.substr(0, hostUrl.length - 1);
    }
    const globalPath = hostUrl;
    if (path && !isHttpUrl(path) && path.indexOf('/') !== 0) {
        path = '/' + path;
    }
    return path ? globalPath + path : globalPath;
};
